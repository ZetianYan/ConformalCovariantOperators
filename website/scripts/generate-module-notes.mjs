...
function meaningFor(name, kind) {
  const special = [
    [/straightForm_X_Q$/, "The Euler field satisfies X(Q)=2Q."],
    [/straightForm_X_laplacian_comm$/, "The Euler field and ambient Laplacian satisfy [X, \\widetilde\\Delta]=-2\\widetilde\\Delta."],
    [/straightForm_lap_Q_comm$/, "The ambient Laplacian of Qf is rewritten as Q\\widetilde\\Delta f plus the homogeneity correction."],
    [/lapPow_isTangentialAtCriticalWeight$/, "The kth ambient Laplacian power descends at the critical weight k-n/2."],
    [/canonicalAmbientGJMS$/, "The descended ambient Laplacian power is packaged as the canonical abstract GJMS operator."],
    [/finitePart/i, "Selects or identifies the constant term in a Laurent/log cutoff expansion."],
    [/logAnomaly/i, "Selects or identifies the logarithmic coefficient in a cutoff expansion."],
    [/formalP_one_eq_expectedP2$/, "The normalized formal recursion agrees with the encoded raw second-order expression."],
    [/formalP_two_eq_expectedP4Raw$/, "The normalized formal recursion agrees with the encoded raw fourth-order expression."],
    [/ClosedSatisfies|closed.*recurrence/i, "The proposed closed coefficient expression satisfies the encoded neighboring-index recurrence."],
  ];
  const hit = special.find(([pattern]) => pattern.test(name));
  if (hit) return hit[1];
  const prefix = kind === "theorem" || kind === "lemma" ? "Formal proposition" : kind === "structure" || kind === "class" ? "Data/interface" : "Definition";
  return `${prefix}: ${words(name)}. The exact mathematical type is preserved from the Lean signature shown alongside it.`;
}
...
for (const full of leanFiles) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const match = lines[i].match(/^\s*(?:noncomputable\s+)?(structure|class|def|abbrev|inductive|theorem|lemma|axiom)\s+([^\s(:{]+)/);
    if (!match) continue;
    let signature = lines[i].trim();
    for (let j = i + 1; j < Math.min(lines.length, i + 7) && !/(:=| where$| := by$)/.test(signature); j++) {
      const next = lines[j].trim();
      if (!next || next.startsWith("--") || next.startsWith("/-")) break;
      signature += ` ${next}`;
      if (/(:=| where$| := by$)/.test(next)) break;
    }
    signature = signature.replace(/\s*:=.*$/, "").replace(/\s+where$/, "").trim();
    correspondence.push({ leanDeclaration: match[2], kind: match[1], mathematicalMeaning: meaningFor(match[2], match[1]), leanType: signature, line: i + 1 });
  }
...
for (const [file, module] of modules) {
  ...
  const note = {
    ...
    mathematicalContent: [
      ...
      { heading: "Formal boundary", body: `This page describes exactly the declarations present in ${file}; claims not represented by a declaration are not counted as verified.`, formula: null },
    ],
    correspondence: module.correspondence.slice(0, 18),
